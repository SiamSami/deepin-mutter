                      ...
                      ClutterInputDevice     *device)
{
  ...
  if (serial == 0 || serial == 1)
    {
      path = g_strdup_printf ("/com/deepin/wrap/gnome/desktop/peripherals/stylus/default-%s:%s/",
                              clutter_input_device_get_vendor_id (device),
                              clutter_input_device_get_product_id (device));
    }
  else
    {
      path = g_strdup_printf ("/com/deepin/wrap/gnome/desktop/peripherals/stylus/%" G_GINT64_MODIFIER "x/",
                              serial);
    }

  tool_settings =
    g_settings_new_with_path ("com.deepin.wrap.gnome.desktop.peripherals.tablet.stylus",
                              path);
                            ...
                            gint                mode)
{
  ...
  g_string_append_printf (path, "/com/deepin/wrap/gnome/desktop/peripherals/tablets/%s:%s/%s%c",
  ...
  settings = g_settings_new_with_path ("com.deepin.wrap.gnome.desktop.peripherals.tablet.pad-button",
                                       path->str);
...
meta_input_settings_init (MetaInputSettings *settings)
{
  ...
  priv->gsd_settings = g_settings_new ("com.deepin.wrap.gnome.settings-daemon.peripherals.mouse");

  g_settings_bind (priv->gsd_settings, "double-click",
                   clutter_settings_get_default(), "double-click-time",
                   G_SETTINGS_BIND_GET);

  priv->keyboard_a11y_settings = g_settings_new ("com.deepin.wrap.gnome.desktop.a11y.keyboard");
  ...
  priv->mouse_a11y_settings = g_settings_new ("com.deepin.wrap.gnome.desktop.a11y.mouse");
